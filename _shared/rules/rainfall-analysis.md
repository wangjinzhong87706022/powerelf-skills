...
> ℹ️ 上表是**描述性等级划分**（报汛用语），不等同于"预警触发"，**不要把等级颜色直接当预警**。
> 单站**发预警的权威口径** = inspection 引擎 `analyze_rainfall`（`ew_info_rules` ew_type=2 缺省兜底，SL 158-2010）：
> **单时段 p 红>100 / 橙>80 / 黄>50 / 蓝>30 mm**。
> 例：单时段 p=100.1mm → **红色预警**（100.1>100）；p=29.9mm 属"大雨"（描述）但 <30 → 不发预警。
> ⚠️ 下方"分析规则"中：强度>16、累计>100mm(3天) 是**辅助分析视角**；p>200 是**数据校验线**（极端值需人工确认）——都**不是**预警触发线，勿混用。
