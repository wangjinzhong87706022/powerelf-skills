...
⚠️ 边界值判定（严格比较，高频踩坑）:
  上述所有阈值都是**严格不等式**（`>`/`<`），**恰等于阈值一律不触发**。
  判定时先做减法取**精确差值**，禁止四舍五入、禁止用 σ/标准差近似替代绝对阈值。
  - 渗压: |wp - prev_wp| = 5.0（恰好5kPa）→ 不触发 `> 5`
    · 50.0 → 54.9，差 4.9 → **不触发**　　50.0 → 55.1，差 5.1 → 触发
  - 水位: |z - prev_z| = 0.5（恰好0.5m）→ 不触发 `> 0.5`
  - 百分比类同理: 变化率恰等于 5%/10%/20% → 不触发
```
