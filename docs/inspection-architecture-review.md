...
|---|---|---|---|---|
| ① | 离线数偏多 | "总离线设备 524 台记录… 42.2% (54/128)" | `deleted=0` 下 `status=0` 实为 **54/128 = 42.2%**（149 为含已删除设备的全表口径） | ❌ 记录/设备混用（524 记录 ≠ 54 设备） |
...
| **N3** | **离线口径混用（修订：非分母过期）** | 42.2%(54/128) 是 `deleted=0` 下的**实时正确值**（inspection `read_equipment` 查询带 `deleted=0`，128 台为正确分母）；149 台含 21 台已删除设备，不构成"分母过期"。真正问题是"524 离线**记录数** vs 54 离线**设备数**"口径混用（`classify_offline_by_duration` LEFT JOIN 展开 + 无窗口）。 |
