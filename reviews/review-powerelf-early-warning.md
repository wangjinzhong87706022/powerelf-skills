...
| Low | 架构 | `SKILL.md:3` frontmatter description | **description 字段长度 171 字符，但只有前 57 字符（`水利工程预警系统：阈值预警、开关量预警、状态变化预警、`)参与 hermes 路由判定**（依据 `mem:hermes-skill-description-truncation`）。前 57 字符已含"阈值预警/开关量预警/状态变化预警" 3 个核心路由词，**但缺**"大坝"（对应 dam-rules，路由核心场景）、"趋势"（对应 trend-rules）、"视频 AI 报警"（§核心数据表 line 30 列出 `ew_camera_info` 表但 description 完全未提）。如果用户问"大坝位移异常"会落到 monitor 而非 early-warning。`metadata.hermes.tags` 也缺 `dam` / `trend` / `video` 标签。 | 改写 description 头部：把"大坝安全预警"和"趋势预警"前置到前 57 字符内（可压缩为"水利工程预警系统：大坝/趋势/阈值/开关量预警"），并在 tags 中加 `dam` / `trend` / `video-ai`。 |
