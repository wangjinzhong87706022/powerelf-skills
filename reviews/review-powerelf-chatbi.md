...
| L2 "关键字黑名单" 声明（query_exec.py:9） | FORBIDDEN_KEYWORDS 16 个词覆盖 DDL/DML 写操作 + call/load/handler/rename/lock/unlock — ⚠️ 遗漏 sleep/benchmark/outfile/dumpfile（见 H1/H2） |
...
> **评审计数**：0 Blocker + 3 High + 6 Medium + 5 Low = **14 findings**（chatbi 是 5 主 skill 中代码密度最高的，finding 数量相应多于 monitor 10 / early-warning 12 / inspection 20 / governance 22）。**核心修复优先级**：H1（黑名单加 sleep/benchmark）→ H2（黑名单加 outfile/dumpfile）→ H3（L5 LIMIT 数值检查 + few_shots JOIN 键修正）→ M1-M6（few_shots 对齐 schema）→ M3（L6/L7 测试补充）。H1/H2 修复仅需在 FORBIDDEN_KEYWORDS frozenset 中加 4 个字符串，影响面最小、安全收益最大。
