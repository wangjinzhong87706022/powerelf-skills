...
> 所有 SQL 均使用 powerelf_srm_yml 前缀库，包含 deleted=0 和 tenant_id=1。
...
FROM powerelf_srm_yml.st_rsvr_r
WHERE st_id = (
  SELECT id FROM powerelf_srm_yml.att_st_base WHERE name LIKE '%XX水库%' AND deleted = 0 LIMIT 1
)
...
JOIN powerelf_srm_yml.att_st_base s ON r.st_id = s.id
...
FROM powerelf_srm_yml.st_pptn_r r
JOIN powerelf_srm_yml.att_st_base s ON r.st_id = s.id
...
FROM powerelf_srm_yml.eq_equip_base
WHERE status = 0
  AND deleted = 0
  AND tenant_id = 1
...
FROM powerelf_srm_yml.eq_equip_base e
JOIN powerelf_srm_yml.eq_equip_offline_record o ON e.code = o.equipment_code
...
FROM powerelf_srm_yml.ew_info_message
WHERE deleted = 0 AND tenant_id = 1
...
SELECT tm AS 采集时间,
       ...
       speed_gh AS 高程速率
FROM powerelf_srm_yml.dsm_dfr_srvrds_srhrds
...
FROM powerelf_srm_yml.st_pressure_r
WHERE section_id = XX
  AND tm > DATE_SUB(NOW(), INTERVAL 7 DAY)
  AND deleted = 0 AND tenant_id = 1
ORDER BY tm
...
FROM powerelf_srm_yml.business_check_error
...
FROM powerelf_srm_yml.business_check_task
...
FROM powerelf_srm_yml.stats_data_collection_daily c
LEFT JOIN powerelf_srm_yml.stats_data_missing_daily m
  ON c.tm = m.tm AND c.table_name = m.table_name AND m.deleted = 0
LEFT JOIN powerelf_srm_yml.stats_data_anomaly_daily a
...
FROM (
  ...
  FROM powerelf_srm_yml.rei_gate_r
  WHERE deleted = 0 AND tenant_id = 1
) r
JOIN powerelf_srm_yml.att_st_base s ON r.st_id = s.id
WHERE r.rn = 1 AND s.deleted = 0
...
FROM (
  SELECT *, ROW_NUMBER() OVER (PARTITION BY st_id ORDER BY tm DESC) AS rn
  FROM powerelf_srm_yml.rei_pump_r
  WHERE deleted = 0 AND tenant_id = 1
) r
JOIN powerelf_srm_yml.att_st_base s ON r.st_id = s.id
WHERE r.rn = 1 AND s.deleted = 0
...
FROM (
  SELECT *, ROW_NUMBER() OVER (PARTITION BY st_id ORDER BY tm DESC) AS rn
  FROM powerelf_srm_yml.st_soil_moisture_r
  WHERE deleted = 0 AND tenant_id = 1
) r
JOIN powerelf_srm_yml.att_st_base s ON r.st_id = s.id
WHERE r.rn = 1 AND s.deleted = 0
...
FROM powerelf_srm_yml.st_termite_monitor_r r
JOIN powerelf_srm_yml.att_st_base s ON r.st_id = s.id
...
FROM powerelf_srm_yml.ew_camera_info c
LEFT JOIN powerelf_srm_yml.att_st_base s ON c.st_id = s.id
...
FROM powerelf_srm_yml.st_percolation_r
WHERE st_id = XX
  AND tm > DATE_SUB(NOW(), INTERVAL 7 DAY)
  AND deleted = 0 AND tenant_id = 1
ORDER BY tm
...
FROM (
  SELECT *, ROW_NUMBER() OVER (PARTITION BY st_id ORDER BY tm DESC) AS rn
  FROM powerelf_srm_yml.st_river_r
  WHERE deleted = 0 AND tenant_id = 1
) r
JOIN powerelf_srm_yml.att_st_base s ON r.st_id = s.id
WHERE r.rn = 1 AND s.deleted = 0
