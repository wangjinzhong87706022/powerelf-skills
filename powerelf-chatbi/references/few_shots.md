...
> 所有 SQL 均包含 deleted=0 和 tenant_id=1，关联键遵循 schema.md 铁律（stcd/eq_id）。
...
FROM ew_camera_info c
LEFT JOIN att_st_base s ON c.st_id = s.id
